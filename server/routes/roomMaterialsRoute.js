...
router.get("/rooms/:roomId/materials", authMiddleware, asyncHandler(async (req, res) => {
...
router.post("/rooms/:roomId/materials", authMiddleware, rules.addRoomMaterial, validate, asyncHandler(async (req, res) => {
    const { roomId } = req.params;
...
router.put("/room-materials/:id", authMiddleware, rules.idParam, validate, asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    await updateRoomAndQuotationTotals(updatedRoomMaterial.room_id);

    res.status(200).json(updatedRoomMaterial);
}));

// DELETE a material from a room
router.delete("/room-materials/:id", authMiddleware, rules.idParam, validate, asyncHandler(async (req, res) => {
    const { id } = req.params;

    const [[item]] = await db.query("SELECT room_id FROM room_items WHERE id = ?", [id]);
    if (!item) return res.status(404).json({ message: "Item not found." });

    await db.query("DELETE FROM room_items WHERE id = ?", [id]);

    await updateRoomAndQuotationTotals(item.room_id);
